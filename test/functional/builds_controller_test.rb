...
class BuildsControllerTest < ActionController::TestCase
  ...
  test "should create build" do
    assert_difference('Build.count') do
      post :create, build: { build_number: @build.build_number, skipped_test: @build.skipped_test, total_test: @build.total_test }
    end
  ...
  test "should update build" do
    put :update, id: @build, build: { build_number: @build.build_number, skipped_test: @build.skipped_test, total_test: @build.total_test }
    assert_redirected_to build_path(assigns(:build))
  end
